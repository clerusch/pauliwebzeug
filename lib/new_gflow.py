...
def gflow(
    ...
    # add ancilla initialisation and measurement as pattern io
    for inp in g.inputs():
        if vertex_is_zx(ty[inp]):
            pattern_inputs |= set([inp])
        else:
            pattern_inputs |= set(n for n in g.neighbors(inp) if vertex_is_zx(ty[n]))
    for outp in g.outputs():
        if vertex_is_zx(ty[outp]):
            pattern_outputs |= set([outp])
        else:
...
def main():
    ...
    # lg = load_graph("./zxgs/pattern_io_hack.zxg")
    # # zx.draw(lg, labels=True)
    # lg.set_inputs((0,2))
    # lg.set_outputs((1,4))
    # flow = gflow(lg, focus=True, reverse=False, pauli=True)
    # print(flow)
    from pyzx.gflow import gflow as gflo2
    c = zx.qasm("""
    qreg q[2];
    z q[1];
    cx q[0], q[1];
