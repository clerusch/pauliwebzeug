...
from new_gflow import gflow
from pyzx.utils import EdgeType, VertexType, vertex_is_zx, phase_is_clifford
...
def cpw(g):
    """
    Compute the Pauli webs of a given graph.
    """
    g1 = g.clone()
    ...
    draw(g1, labels=True)
    gf = gflow(g1, focus=True, reverse=True, pauli=True)
    print(gf)
    if not gf:
